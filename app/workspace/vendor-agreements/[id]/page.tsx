...
// Staff-facing vendor agreement detail page — mirrors the admin CRM vendor
// agreement detail page (app/crm/vendor-agreements/[id]/page.tsx) exactly
// in look, feel and feature set (Download, Print, Email, WhatsApp,
// Duplicate, Revise, performance stats, PDF Preview, Activity, Versions,
// Documents) so staff who have vendor agreement access get the same
// experience admins do. Route access is already gated by middleware.ts via
// the "vendorAgreements" module toggle in Manage Access, same as the rest
// of /workspace/vendor-agreements/* — no separate permission logic needed.
...
} from '../../../crm/vendor-agreements/vendor-agreement-config';
...
export default function WorkspaceVendorAgreementDetailPage() {
  ...
  const printPdf = async () => {
    ...
    await appendVendorAgreementActivity(agreement.id, {
      type: 'printed', title: 'Vendor agreement printed', detail: `Version ${agreement.version} opened for printing.`, actor: 'CRM Staff',
    });
    window.print();
  };
  ...
  const duplicate = async () => {
    ...
    try {
      ...
      router.push(`/workspace/vendor-agreements/${copy.id}/edit`);
    } finally {
      setBusy('');
    }
  };
  ...
  const emailAgreement = async () => {
    ...
    await appendVendorAgreementActivity(agreement.id, {
      type: 'sent', title: 'Email prepared', detail: `Email draft opened for ${agreement.email || agreement.vendor_name}.`, actor: 'CRM Staff',
    });
  ...
  const whatsappAgreement = async () => {
    ...
    await appendVendorAgreementActivity(agreement.id, {
      type: 'sent', title: 'WhatsApp share opened', detail: `Vendor agreement share prepared for ${agreement.mobile}.`, actor: 'CRM Staff',
    });
  ...
  const addDocument = async (file: UploadedFile) => {
    ...
    setAgreement(updated);
    await appendVendorAgreementActivity(agreement.id, { type: 'document', title: 'Document uploaded', detail: `${docCategory}: ${file.file_name}`, actor: 'CRM Staff' });
  };
  ...
  if (!agreement) {
    return (
      <>
        <CrmHeader title="Vendor Agreement" subtitle="Loading vendor agreement" onMenuClick={open} notificationsHref="/workspace/notifications" />
      ...
      <CrmHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            <Link href="/workspace/vendor-agreements" className="hidden items-center gap-1.5 text-xs font-bold text-gray-400 hover:text-gray-900 xl:inline-flex"><ArrowLeft size={14} /> Vendor Agreements</Link>
            <Link href={`/workspace/vendor-agreements/${agreement.id}/edit`} className="inline-flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-3 py-2 text-xs font-bold text-gray-700 hover:bg-gray-50"><Pencil size={14} /> <span className="hidden sm:inline">Edit</span></Link>
          </div>
        }
      />
      ...
      <div className="space-y-5 p-4 sm:p-6">
        <div className="grid gap-4 xl:grid-cols-[1fr_auto]">
          ...
          <div className="grid grid-cols-3 gap-2 sm:grid-cols-6 xl:grid-cols-3">
            ...
            <Link href={`/workspace/vendor-agreements/${agreement.id}/edit`} className="agreement-action-button"><Pencil size={17} />Revise</Link>
          </div>
        </div>
