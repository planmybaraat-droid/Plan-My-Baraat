...
export default function WorkspaceQuotationDetailPage() {
  ...
  const download = async () => { if (!quotation || !documentRef.current) return; setBusy('download'); try { await downloadCrmPdf(documentRef.current, `${quotation.quotation_number}-${quotation.client_name.replace(/[^a-z0-9]+/gi, '-')}.pdf`, { properties: { title: `${quotation.quotation_number} - ${quotation.client_name}`, subject: 'PlanMyBaraat Client Quotation' } }); notify('Quotation PDF downloaded.'); } catch (error) { notify(error instanceof Error ? error.message : 'PDF export failed.'); } finally { setBusy(''); } };
