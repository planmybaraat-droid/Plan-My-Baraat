...
export default function MyQuotationsPage() {
  ...
  return (
    <>
      <CrmHeader title="My Quotations" subtitle={`${rows.length} quotations`} onMenuClick={open} notificationsHref="/workspace/notifications" actions={<Link href="/workspace/quotations/new" className="inline-flex items-center gap-2 rounded-xl bg-red-600 px-3.5 py-2 text-xs font-bold text-white"><Plus size={15}/><span className="hidden sm:inline">New quotation</span><span className="sm:hidden">New</span></Link>} />
      <div className="p-4 sm:p-6">
        <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
          {loading ? (
            ...
            <div className="px-6 py-20 text-center"><FileText className="mx-auto text-red-600" size={28} /><p className="mt-4 font-black">No quotations yet</p><p className="mt-1 text-sm text-gray-400">Quotations assigned to you, or that you create, will appear here — and your admin sees them instantly too.</p><Link href="/workspace/quotations/new" className="mt-5 inline-block rounded-xl bg-red-600 px-5 py-2.5 text-xs font-bold text-white">Create your first quotation</Link></div>
          ) : (
