...
export default function MyAgreementsPage() {
  ...
  return (
    <>
      <CrmHeader title="My Agreements" subtitle={`${rows.length} agreements`} onMenuClick={open} notificationsHref="/workspace/notifications" />
      <div className="p-4 sm:p-6">
        <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
          {loading ? (
            ...
            <div className="px-6 py-20 text-center"><ScrollText className="mx-auto text-red-600" size={28} /><p className="mt-4 font-black">No agreements yet</p><p className="mt-1 text-sm text-gray-400">Agreements assigned to you, or that you create, will appear here — and your admin sees them instantly too.</p></div>
          ) : (
            <div className="divide-y divide-gray-100">
              {rows.map((a) => (
                <div key={a.id} className="flex flex-wrap items-center justify-between gap-3 px-4 py-4 sm:px-6">
                  ...
                  <span className={`rounded-full px-2.5 py-1 text-[9px] font-black uppercase ${STATUS_STYLE[a.status] || 'bg-gray-100 text-gray-600'}`}>{a.status}</span>
                </div>
              ))}
            </div>
          )}
