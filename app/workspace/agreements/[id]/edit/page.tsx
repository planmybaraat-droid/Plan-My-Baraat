...
export default function EditWorkspaceAgreementPage() {
  ...
  const save = async (data: AgreementFormData) => { await updateAgreement(params.id, data); router.push('/workspace/agreements'); };
  return <><CrmHeader title="Edit Agreement" subtitle={record?.agreement_number || 'Loading agreement'} onMenuClick={open} notificationsHref="/workspace/notifications" actions={<Link href="/workspace/agreements" className="inline-flex items-center gap-2 text-xs font-bold text-gray-500"><ArrowLeft size={15}/>Agreements</Link>}/><div className="p-4 sm:p-6">{record ? <AgreementForm initialData={record} onSubmit={save} onSaveDraft={save} submitLabel="Save changes"/> : error ? <div className="flex items-center gap-2 rounded-xl border border-red-200 bg-red-50 p-4 text-sm font-semibold text-red-700"><CircleAlert size={18}/>{error}</div> : <div className="flex h-72 items-center justify-center"><Loader2 className="animate-spin text-red-600"/></div>}</div></>;
}
