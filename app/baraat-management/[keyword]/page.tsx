...
export function generateStaticParams() {
  return Object.keys(BARAAT_KEYWORD_CONTENT).map((slug) => ({ keyword: slug }));
}
...
export async function generateMetadata({
  ...
  const kw = getBaraatKeywordBySlug(params.keyword);
  const content = BARAAT_KEYWORD_CONTENT[params.keyword];
  if (!kw || !content) return {};

  const title = capitalizePhrase(kw.phrase);
  const description = `${title} — PlanMyBaraat runs baraat on wheels packages across Gujarat: DJ truck, dhol team, vintage car, and safa styling, booked as one package.`;
  const canonical = `/baraat-management/${params.keyword}`;
...
export default function KeywordPage({
  ...
  if (!kw || !content) notFound();

  const title = capitalizePhrase(kw.phrase);

  const waText = encodeURIComponent(
    `Hi PlanMyBaraat!\n\nI'm searching for "${kw.phrase}". Please share package details and availability.`
  );
  const waLink = `https://wa.me/${WHATSAPP_NUMBER}?text=${waText}`;

  const breadcrumbItems = [
    { name: "Home", url: BASE_URL },
    { name: title, url: `${BASE_URL}/baraat-management/${kw.slug}` },
  ];

  const jsonLdBreadcrumb = generateJsonLdBreadcrumbGeneric(breadcrumbItems);
  const jsonLdService = generateJsonLdServiceGeneric({
    name: title,
    description: `${title} — DJ truck, dhol team, vintage car, and safa styling for baraat processions across Gujarat.`,
    areaServedName: "Gujarat",
    url: `${BASE_URL}/baraat-management/${kw.slug}`,
  });
  const jsonLdFaq = generateJsonLdFAQGeneric(content.faqs);

  const sections: Array<{ eyebrow: string; heading: string; body: string }> = [
    { eyebrow: "What This Means", heading: `About "${kw.phrase}"`, body: content.explanation },
    ...
    { eyebrow: "Booking", heading: "How to book", body: content.bookingNotes },
  ];

  const relatedKeywords = BARAAT_KEYWORDS.filter(
    (k) => k.category === kw.category && k.slug !== kw.slug && Boolean(BARAAT_KEYWORD_CONTENT[k.slug])
  );
        ...
        <section className="contact-hero">
          ...
          <div className="relative z-10 mx-auto max-w-7xl px-5 py-16 sm:px-8 sm:py-20 lg:px-10 lg:py-24">
            <div className="mx-auto max-w-2xl text-center">
              ...
              <h1 className="text-[clamp(1.85rem,4.6vw,3.15rem)] font-extrabold leading-[1.05] tracking-[-0.038em] text-white">
                {title}
              </h1>
              <p className="mx-auto mt-5 max-w-xl text-sm leading-relaxed text-white/65 sm:text-base">
                ...
                booked as one package for your baraat entry.
              </p>
            </div>
        ...
        {relatedKeywords.length > 0 ? (
          <section className="border-t border-[#010101]/10 bg-white px-5 py-12 sm:px-8 lg:px-10">
            <div className="mx-auto max-w-4xl">
              <SeoLinkBlock
                title={`More on ${kw.category}`}
                summary={`${relatedKeywords.length} related searches people use.`}
                items={relatedKeywords.map((k) => ({
                  label: capitalizePhrase(k.phrase),
                  href: `/baraat-management/${k.slug}`,
                }))}
              />
            </div>
          </section>
