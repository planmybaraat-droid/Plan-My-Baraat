...
function normalizeAttendance(row: Record<string, unknown>): AttendanceRecord {
  return { id: String(row.id || ''), staff_id: String(row.staff_id), attendance_date: String(row.attendance_date), status: row.status as AttendanceRecord['status'], check_in: String(row.check_in || '').slice(0, 5), check_out: String(row.check_out || '').slice(0, 5), break_minutes: Number(row.break_minutes || 0), overtime_minutes: Number(row.overtime_minutes || 0), note: String(row.note || ''), punch_in_at: row.punch_in_at ? String(row.punch_in_at) : null, punch_out_at: row.punch_out_at ? String(row.punch_out_at) : null, late_minutes_at_punch_in: Number(row.late_minutes_at_punch_in || 0), late_reason: row.late_reason ? String(row.late_reason) : null, late_reason_status: row.late_reason_status as AttendanceRecord['late_reason_status'], late_reason_review_note: row.late_reason_review_note ? String(row.late_reason_review_note) : null, late_reason_reviewed_by: row.late_reason_reviewed_by ? String(row.late_reason_reviewed_by) : null, late_reason_reviewed_at: row.late_reason_reviewed_at ? String(row.late_reason_reviewed_at) : null, created_at: String(row.created_at || ''), updated_at: String(row.updated_at || ''), breaks: Array.isArray(row.breaks) ? row.breaks as AttendanceBreakRecord[] : [], staff: row.staff ? normalizeStaff(row.staff as Record<string, unknown>) : undefined };
}
