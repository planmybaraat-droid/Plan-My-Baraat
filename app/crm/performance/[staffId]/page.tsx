...
export default function PerformanceDetailPage() {
  ...
  const nonWorkingDayOvertime = result
    ...
    : 0;
  const weekdayOvertime = result
    ? Math.max(0, result.totalOvertimeMinutes - nonWorkingDayOvertime)
    : 0;
  return (
    <>
      <CrmHeader
        ...
        {loading ? (
          ...
          <>
            ...
            <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              ...
              <TimingValue
                icon={<Zap size={16} />}
                label="Sunday & holiday overtime"
                value={formatPerformanceMinutes(nonWorkingDayOvertime)}
                note={`${formatPerformanceMinutes(weekdayOvertime)} on regular workdays`}
                tone="red"
              />
            </section>
