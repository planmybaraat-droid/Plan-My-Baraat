...
export async function loadPerformance(options: {
  ...
  const results = staff.map((person) => {
    ...
    for (const date of datesBetween(options.start, options.end)) {
      ...
      // A punch on any configured non-working day is real overtime. This
      // includes the normal Sunday weekly off as well as company holidays.
      const workedNonWorkingDay = (holiday || weeklyOff) && hasPunch;
      const excluded =
        date < first ||
        (weeklyOff && !workedNonWorkingDay) ||
        (holiday && !workedNonWorkingDay) ||
        leave ||
        (["holiday", "weekly off", "on leave"].includes(status) && !workedNonWorkingDay);
      ...
      if (current && !hasOut) {
        days.push({
          ...
          attendance: workedNonWorkingDay
            ? holiday
              ? "Holiday Work"
              : "Weekly Off Work"
            : hasPunch
      ...
      if (workedNonWorkingDay) {
        ...
        days.push({
          ...
          attendance: holiday ? "Holiday Work" : "Weekly Off Work",
          punchIn: row?.check_in || null,
          punchOut: row?.check_out || null,
          workingMinutes: net,
          requiredMinutes: 0,
          ...
          report: null,
          reason: hasOut
            ? `${overtimeMinutes} ${holiday ? "holiday" : "weekly-off"} overtime minutes`
            : `${holiday ? "Holiday" : "Weekly-off"} work is awaiting punch out`,
        });
        continue;
      }
      eligible++;
