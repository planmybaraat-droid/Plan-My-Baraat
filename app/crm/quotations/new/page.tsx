...
export default function NewQuotationPage() {
  ...
  return <><CrmHeader title="Create Quotation" subtitle="Build a tailored package from the Agreement service catalogue" onMenuClick={open} actions={<Link href="/crm/quotations" className="inline-flex items-center gap-2 text-xs font-bold text-gray-500"><ArrowLeft size={15} /> Quotations</Link>} /><div className="p-4 sm:p-6">{initialData ? <QuotationForm initialData={initialData} onSubmit={submit} submitLabel="Generate quotation" /> : error ? <div className="mx-auto flex max-w-2xl items-start gap-3 rounded-2xl border border-red-200 bg-red-50 p-5 text-sm font-semibold text-red-700"><CircleAlert className="shrink-0" size={18}/><div><p>{error}</p><button onClick={() => window.location.reload()} className="mt-3 rounded-lg bg-red-600 px-3 py-2 text-xs font-bold text-white">Try again</button></div></div> : <div className="flex h-72 items-center justify-center"><Loader2 className="animate-spin text-red-600" /></div>}</div></>;
}
