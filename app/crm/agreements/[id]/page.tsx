...
export default function AgreementDetailPage() {
  ...
  const printPdf = async () => {
    ...
    await appendAgreementActivity(agreement.id, {
      type: 'printed', title: 'Agreement printed', detail: `Version ${agreement.version} opened for printing.`, actor: agreement.sales_executive || 'CRM Admin',
    });
    window.print();
  };
  ...
  const emailAgreement = async () => {
    ...
    await appendAgreementActivity(agreement.id, {
      type: 'sent', title: 'Email prepared', detail: `Email draft opened for ${agreement.email || agreement.client_name}.`, actor: agreement.sales_executive || 'CRM Admin',
    });
  ...
  const whatsappAgreement = async () => {
    ...
    await appendAgreementActivity(agreement.id, {
      type: 'sent', title: 'WhatsApp share opened', detail: `Agreement share prepared for ${agreement.mobile}.`, actor: agreement.sales_executive || 'CRM Admin',
    });
  ...
  if (!agreement) {
    return (
      <>
        <CrmHeader title="Agreement" subtitle="Loading client agreement" onMenuClick={open} />
      ...
      <CrmHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            <Link href="/crm/agreements" className="hidden items-center gap-1.5 text-xs font-bold text-gray-400 hover:text-gray-900 xl:inline-flex"><ArrowLeft size={14} /> Agreements</Link>
            <Link href={`/crm/agreements/${agreement.id}/edit`} className="inline-flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-3 py-2 text-xs font-bold text-gray-700 hover:bg-gray-50"><Pencil size={14} /> <span className="hidden sm:inline">Edit</span></Link>
          </div>
        }
      />
      ...
      <div className="space-y-5 p-4 sm:p-6">
        <div className="grid gap-4 xl:grid-cols-[1fr_auto]">
          ...
          <div className="grid grid-cols-3 gap-2 sm:grid-cols-7 xl:grid-cols-4">
            <Link href={`/crm/invoices/new?agreementId=${agreement.id}`} className="agreement-action-button bg-red-600 text-white"><FileText size={17} />Invoice</Link>
            ...
            <Link href={`/crm/agreements/${agreement.id}/edit`} className="agreement-action-button"><Pencil size={17} />Revise</Link>
          </div>
        </div>
        ...
        {tab === 'attachments' && (
          <div className="grid gap-5 lg:grid-cols-[360px_1fr]">
            <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
              ...
              <FileUploader entityType="agreement" entityId={agreement.id} onUploadComplete={file => { setFiles(current => [file, ...current]); appendAgreementActivity(agreement.id, { type: 'attachment', title: 'Attachment added', detail: file.file_name, actor: agreement.sales_executive || 'CRM Admin' }); }} />
            </div>
            <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
