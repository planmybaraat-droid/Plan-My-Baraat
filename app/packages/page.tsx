...
import { SIGNATURE_PACKAGE as pkg } from "@/lib/packagesData";
import {
...
export const metadata: Metadata = {
  title: pkg.name,
  description: `Explore the ${pkg.name}: included entry, sound, dhol, lighting, effects, Safa support, selectable experiences and planning guidance.`,
  alternates: { canonical: "/packages" },
  openGraph: {
    title: `${pkg.name} | Plan My Baraat`,
  ...
  twitter: { card: "summary_large_image", title: pkg.name, description: pkg.description, images: [pkg.image] },
};
...
const JOURNEY = [
  ...
  { number: "02", title: "Personalize the plan", body: "Your Baraat on Wheels, groom entry, and selectable experiences are finalized together." },
  ...
  { number: "04", title: "Execute the Baraat", body: "Our dedicated on-ground team manages the procession cues so your family can participate instead of coordinating vendors." },
];
...
export default function PackagesPage() {
  ...
  const quickFacts = [
    { icon: Sparkles, label: "Essentials included", value: `${pkg.essentials.length} must-have services` },
    { icon: Users, label: "Selectable experiences", value: `${pkg.selectableExperiences.length} to choose from` },
    { icon: CalendarCheck, label: "Execution team", value: "30-50 dedicated members" },
  ];
      ...
      <div className="inner-public-page relative flex min-h-screen flex-col bg-white font-sans text-[#010101]">
        ...
        <main className="flex-grow">
            ...
            <div className="relative mx-auto flex min-h-[38rem] max-w-7xl items-end px-5 pb-14 pt-32 sm:px-8 sm:pb-16 lg:min-h-[44rem] lg:px-10 lg:pb-20">
              <div className="max-w-3xl">
                ...
                <div className="mt-9 flex flex-col gap-3 sm:flex-row">
                  ...
                  <a href={pkg.pdfUrl} download className="inline-flex min-h-12 items-center justify-center gap-2.5 rounded-xl border border-white/22 bg-white/[0.06] px-6 text-sm font-extrabold text-white backdrop-blur-sm transition-colors hover:bg-white hover:text-[#010101]"><Download className="h-4 w-4" aria-hidden="true" />Download package PDF</a>
                </div>
              </div>
            </div>
          </section>
          ...
          <section className="relative z-10 mx-auto -mt-7 max-w-7xl px-5 sm:px-8 lg:px-10">
            <div className="grid overflow-hidden rounded-2xl border border-[#010101]/10 bg-white shadow-[0_30px_80px_-62px_rgba(1,1,1,0.75)] sm:grid-cols-3">
              {quickFacts.map(({ icon: Icon, label, value }) => (
                <div key={label} className="flex min-h-32 gap-4 border-b border-[#010101]/10 p-5 last:border-b-0 sm:border-b-0 sm:border-r sm:p-6 sm:last:border-r-0">
          ...
          <section className="mx-auto grid max-w-7xl gap-12 px-5 py-20 sm:px-8 sm:py-24 lg:grid-cols-[1.08fr_0.92fr] lg:gap-20 lg:px-10 lg:py-28">
            ...
            <Reveal delay={1}>
              <aside className="rounded-2xl bg-[#010101] p-6 text-white sm:p-8 lg:sticky lg:top-28">
                ...
                <a href={pkg.pdfUrl} download className="mt-8 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-white px-4 text-xs font-extrabold text-[#010101] transition-colors hover:bg-[#E30B1D] hover:text-white"><Download className="h-4 w-4" aria-hidden="true" />Download package PDF</a>
              </aside>
            </Reveal>
          </section>
            ...
            <div className="mx-auto max-w-7xl px-5 py-20 sm:px-8 sm:py-24 lg:px-10 lg:py-28">
              <Reveal>
                <div className="grid gap-6 lg:grid-cols-[0.82fr_1.18fr] lg:items-end">
                  <div><p className="text-[10px] font-extrabold uppercase tracking-[0.16em] text-[#E30B1D]">Essentials: must-haves for every Baraat</p><h2 className="mt-4 text-[clamp(2rem,4.2vw,3.5rem)] font-extrabold leading-[1.02] tracking-[-0.05em]">Included in every {pkg.shortName}.</h2></div>
                  <p className="max-w-2xl text-sm leading-7 text-[#010101]/55 sm:text-base">Nothing is hidden behind vague labels. These are the working essentials our team coordinates for every {pkg.shortName}, with suitable personalization confirmed for your venue and route.</p>
                </div>
              </Reveal>
              <div className="mt-12 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                {pkg.essentials.map((feature, index) => {
          ...
          <section className="border-y border-[#010101]/10 bg-white">
            <div className="mx-auto max-w-7xl px-5 py-20 sm:px-8 sm:py-24 lg:px-10 lg:py-28">
              <Reveal>
                <div className="grid gap-6 lg:grid-cols-[0.82fr_1.18fr] lg:items-end">
                  <div><p className="text-[10px] font-extrabold uppercase tracking-[0.16em] text-[#E30B1D]">Selectable experiences: choose your celebration style</p><h2 className="mt-4 text-[clamp(2rem,4.2vw,3.5rem)] font-extrabold leading-[1.02] tracking-[-0.05em]">Personalize it exactly to your event.</h2></div>
                  <p className="max-w-2xl text-sm leading-7 text-[#010101]/55 sm:text-base">Pick the experiences that fit your celebration and we build them into your quote alongside the essentials above.</p>
                </div>
              </Reveal>
              <div className="mt-12 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                {pkg.selectableExperiences.map((feature, index) => {
                  const { title, detail } = featureParts(feature);
                  return <Reveal key={feature} delay={index % 3}><article className="flex h-full gap-4 rounded-2xl border border-[#010101]/10 bg-white p-5 transition-colors hover:border-[#E30B1D]/30 sm:p-6"><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#E30B1D]/10 text-[#E30B1D]"><Check className="h-4 w-4" strokeWidth={2.6} aria-hidden="true" /></span><div><h3 className="text-sm font-extrabold leading-5">{title}</h3>{detail ? <p className="mt-2 text-xs leading-5 text-[#010101]/50">{detail}</p> : null}</div></article></Reveal>;
                })}
              </div>
            </div>
          </section>
            ...
            <div className="mx-auto grid max-w-7xl gap-10 px-5 py-16 sm:px-8 sm:py-20 lg:grid-cols-[0.78fr_1.22fr] lg:gap-16 lg:px-10 lg:py-24">
              <Reveal>
                <div className="lg:sticky lg:top-28">
                  ...
                  <p className="mt-5 max-w-lg text-sm leading-7 text-[#010101]/58 sm:text-base">Choose the experiences that matter, share your event details, and continue directly on WhatsApp. Our team will confirm compatibility and prepare your personalized quote.</p>
                </div>
              </Reveal>
          ...
          <section className="bg-white">
            <div className="mx-auto grid max-w-7xl gap-10 px-5 py-20 sm:px-8 sm:py-24 lg:grid-cols-[0.82fr_1.18fr] lg:items-start lg:gap-16 lg:px-10 lg:py-28">
              <Reveal><div className="lg:sticky lg:top-28"><p className="text-[10px] font-extrabold uppercase tracking-[0.16em] text-[#E30B1D]">Check your date</p><h2 className="mt-4 text-[clamp(2rem,4.2vw,3.5rem)] font-extrabold leading-[1.02] tracking-[-0.05em]">Is the {pkg.shortName} right for your event?</h2><p className="mt-5 max-w-lg text-sm leading-7 text-[#010101]/55 sm:text-base">Share the essential details. Our team will check availability, understand your route and recommend the right selectable experiences without showing generic pricing.</p><div className="mt-8 flex flex-col gap-3 sm:flex-row lg:flex-col xl:flex-row"><a href={waLink} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[#E30B1D] px-6 text-sm font-extrabold text-white hover:bg-[#010101]"><MessageCircle className="h-4 w-4" aria-hidden="true" />WhatsApp our team</a><EnquireNowButton packageName={pkg.name} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl border border-[#010101]/12 px-6 text-sm font-extrabold hover:border-[#E30B1D] hover:text-[#E30B1D]">Request a callback <ArrowRight className="h-4 w-4" aria-hidden="true" /></EnquireNowButton></div></div></Reveal>
